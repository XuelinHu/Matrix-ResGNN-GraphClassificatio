...
To test residual topology under controlled structural class granularity, we add a synthetic benchmark based on five random-graph families: Erdős--Rényi (ER), Barabási--Albert (BA), stochastic block model (SBM), Watts--Strogatz (WS), and random regular graphs. For each family, we generate seven multi-class datasets with class counts \(C\in\{2,3,4,5,6,7,8\}\). Each class contains 200 graphs, so each dataset contains \(200C\) graphs. Graph sizes are sampled between 30 and 80 nodes, and every node receives an eight-dimensional feature vector derived from normalized degree and random feature channels. The features are intentionally generic and do not encode the class label directly.

The synthetic labels are controlled by family-specific structural parameters. ER classes differ by Bernoulli edge probability, BA classes differ by the attachment count, SBM classes differ by community separation, WS classes differ by rewiring probability, and random regular classes differ by fixed degree. These settings produce five complementary structural regimes: sparse random connectivity, long-tailed degree structure, community structure, local clustering with short paths, and degree-uniform structure. They are used as stress tests rather than as replacements for real graph-classification benchmarks.
...
Synthetic stress test & ER, BA, SBM, WS, and regular graphs with \(C=2,\ldots,8\), 200 graphs per class \\
