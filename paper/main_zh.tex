...
\textbf{方法：} 本文提出矩阵残差图神经网络（Matrix-Residual Graph Neural Networks），将其作为图分类中残差拓扑的受控研究框架。模型族比较五种信息流设计：普通消息传递、纵向残差复用、横向分支交换、局部矩阵残差复用，以及稀疏/门控矩阵残差复用。主实验覆盖六个数据集（PROTEINS、DD、ENZYMES、MUTAG、AIDS 和 Mutagenicity）、四种消息传递算子（GCNConv、GATConv、SAGEConv 和 GINConv）、五类模型和五折图级评估。进一步地，本文进行分支数消融、fold-0 灵敏度扫描、选定 MatrixResGated 候选配置的五折复验、机制分析，并补充一个受控合成多分类 benchmark，覆盖五类随机图族和 2 至 8 个结构类别。

\textbf{结果：} 没有一种残差拓扑在所有数据集和算子上都占优。在 24 个数据集--算子组合中，MatrixRes 获得最多胜出次数（9/24）和最佳平均排名，其次是 MatrixResGated。在 GCNConv 下，不同数据集的优胜模型不同：PROTEINS 上 Plain 最强，DD 上 HorizontalRes 最强，ENZYMES 上 MatrixResGated 最强，MUTAG、AIDS 和 Mutagenicity 上 MatrixRes 最强。分支数消融呈现数据集依赖的有效工作区间，而不是单调容量收益。在合成 benchmark 中，简单基线在二分类和三分类任务上仍然有竞争力；但随着结构类别数增加，矩阵式复用更加有利：在四至八分类区间内，MatrixResGated 相比 Plain 将平均准确率从 0.6827 提升到 0.6943，MatrixRes 从二分类到八分类的准确率下降幅度最小。
...
本文围绕四个研究问题展开：
\begin{enumerate}
    \item \textbf{在共享图分类协议下，哪种残差拓扑最强？} 在保持其他流程固定的前提下，比较普通消息传递、纵向残差复用、横向分支交换、局部矩阵残差复用和稀疏/门控矩阵残差复用。
    \item \textbf{分支数如何改变残差行为？} 检验额外分支是否单调提升准确率，还是只在某一有效工作区间内有益，并将这些趋势与多样性、相似度、梯度和残差流量联系起来。
    \item \textbf{受控矩阵复用在什么情况下有帮助？} 检验矩阵残差流量应当密集传递还是被过滤，并检验候选残差控制设置在完整五折复验后是否仍然成立。
    \item \textbf{当结构类别粒度增加时，残差拓扑如何变化？} 在二至八分类的合成图族上测试相同残差族，并使用准确率、macro-F1 和机会归一化准确率比较不同类别数下的表现。
\end{enumerate}

主要贡献如下：
\begin{itemize}
    \item \textbf{提出图分类残差模型的分支--层表述。} 纵向、横向、矩阵和稀疏/门控矩阵残差族被统一表达为同一网格上的局部邻域，使其信息流差异更加明确。
    \item \textbf{对残差拓扑而非新消息传递算子进行受控评估。} benchmark 保持分类器、readout、折划分协议和 backbone 选择一致，使性能差异能够归因于残差路由。
    \item \textbf{从机制角度解释分支扩展。} 本文将准确率与分支多样性、分支余弦相似度、分支 CKA、梯度范数和残差活跃度联合分析，说明额外分支何时产生有用分离、何时只增加冗余流量。
    \item \textbf{构建受控多分类结构压力测试。} 使用 ER、BA、SBM、WS 和随机正则图族，检验当结构类别数从二类增加到八类时，矩阵式复用是否仍然有用。
    \item \textbf{为残差控制设置提供保守验证协议。} 灵敏度扫描仅用于发现候选设置，选出的候选必须跨折复验后才被视为经验证据。
\end{itemize}
...
为在受控结构类别粒度下测试残差拓扑，本文加入基于五类随机图族的合成 benchmark：Erdős--Rényi（ER）、Barabási--Albert（BA）、随机块模型（SBM）、Watts--Strogatz（WS）和随机正则图。对于每个图族，生成七个多分类数据集，类别数为 \(C\in\{2,3,4,5,6,7,8\}\)。每个类别包含 200 个图，因此每个数据集包含 \(200C\) 个图。图规模在 30 至 80 个节点之间采样，每个节点具有八维特征，特征由归一化度和随机特征通道组成。节点特征有意保持通用，不直接编码类别标签。

合成标签由图族特定结构参数控制。ER 类别由 Bernoulli 边概率区分，BA 类别由 attachment count 区分，SBM 类别由社区分离度区分，WS 类别由重连概率区分，随机正则图类别由固定度区分。这些设置对应五类互补结构状态：稀疏随机连接、长尾度结构、社区结构、局部聚类与短路径、以及度均匀结构。它们被用作残差拓扑压力测试，而不是替代真实图分类 benchmark。
...
默认隐藏维度 & 64 \\
评估方式 & 五折图分类 \\
合成压力测试 & ER、BA、SBM、WS 和随机正则图，\(C=2,\ldots,8\)，每类 200 个图 \\
分支数消融 & \(B=1,\ldots,8\) \\
...
除非另有说明，所有结果均报告五折平均最佳测试准确率 \(\pm\) 折级标准差。真实数据实验使用 LATEST 版本的记录汇总，合成多分类 benchmark 使用 SYN\_MULTI\_FULL 版本的记录汇总。主 benchmark 包含 600 个已完成运行，合成多分类 benchmark 包含 3,500 个已完成运行。
...
表~\ref{tab:main_benchmark} 报告默认分支数 \(B=3\) 下完整 GCNConv 切片。该受控切片有助于固定算子，仅改变残差拓扑。Plain 在 PROTEINS 上最强，HorizontalRes 在 DD 上最强，MatrixResGated 在 ENZYMES 上最强，MatrixRes 在 MUTAG、AIDS 和 Mutagenicity 上最强。
...
分支数具有非单调影响。表~\ref{tab:branch_best} 报告分支数消融中每种拓扑的最佳和最差分支数。在 PROTEINS 上，有效区域是中等分支数：HorizontalRes 在 \(B=4\) 达峰，MatrixRes 在 \(B=6\) 达峰，MatrixResGated 在 \(B=3\) 达峰。在 DD 上，有效区域更小：HorizontalRes 和 MatrixRes 在 \(B=2\) 达峰，而 MatrixResGated 在 \(B=1\) 时已经最强。
...
\caption{HorizontalRes、MatrixRes 和 MatrixResGated 在 PROTEINS 与 DD 上的分支数消融。准确率只在有效工作区间内提升，随后饱和或下降。}
...
为解释分支扩展为何先有益后有害，本文分析分支多样性、分支余弦相似度、分支 CKA 相似度、代表性梯度范数、残差计数总量、活跃比例和门控值。表~\ref{tab:mechanism_snapshot} 报告每个消融目标在最佳分支数处的机制指标。多样性衡量分支表示之间的距离；分支余弦和分支 CKA 衡量表示冗余或对齐；梯度范数刻画优化信号；残差计数和活跃比例描述注入模型的残差流量。
...
为区分候选发现和最终评估，选定的 MatrixResGated 设置被重新运行五折。表~\ref{tab:tuned_candidates} 报告这些后续复验。最强实践候选是 \texttt{DD\_dim\_32}，它将参数量从 42,371 降至 15,043，同时保持竞争力。较低学习率接近默认 MatrixResGated benchmark，较低门控初始化较弱。在 PROTEINS 上，\(\lambda=0.02\) 没有在五折中保持其 fold-0 优势。
...
\textbf{回答：} 稀疏/门控矩阵复用是残差流量控制机制。它是密集 MatrixRes 的补充，而非严格替代；候选设置必须跨折验证后才应被视为性能声明。当前证据支持 DD 上较小的受控矩阵模型，并将 PROTEINS 稀疏化保留为有前景但尚不稳定的方向。

\subsection{RQ4：结构类别粒度增加时会发生什么？}
...
分布级合成结果进一步细化了这一结论。在高类别 BA、ER 和随机正则图设置中，MatrixRes 最强，这些任务的类别标签分别与度增长、边概率或度均匀结构变化有关。其优势在 SBM 上较弱，在 WS 上不存在；社区结构和小世界重连带来了不同的归纳压力。因此，合成 benchmark 支持的是条件性结论：当任务需要区分多个相关结构状态时，矩阵式残差复用最有用，但它并不能解决所有图分布。

\textbf{回答：} 合成类别扩展实验表明，MatrixRes 和 MatrixResGated 在更细粒度结构判别中更有用，尤其体现在四分类到八分类区间。该效果在若干受控的度相关和密度相关图族上最强，但仍然依赖图分布。
...
本文使用以下汇总文件作为结果依据。主 benchmark 汇总包含 120 个模型级行：六个数据集、四种算子、五个模型族，以及每个组合的五折聚合结果。
\begin{itemize}
...
结论 & 主要证据 \\
\midrule
MatrixRes 是整体最强拓扑，但不是通用赢家。 & 表~\ref{tab:operator_wins}、表~\ref{tab:winners_by_operator} 和图~\ref{fig:model_win_counts}。 \\
在固定算子下，GCNConv 结果仍然依赖数据集。 & 表~\ref{tab:main_benchmark} 和图~\ref{fig:main_benchmark}。 \\
分支数呈现“先升后降”，不是单调改进。 & 表~\ref{tab:branch_best} 和图~\ref{fig:branch_ablation}。 \\
有效分支扩展需要表示分离，同时避免过量残差流量。 & 表~\ref{tab:mechanism_snapshot} 和图~\ref{fig:mechanism}。 \\
fold-0 灵敏度不应被当作最终证据。 & 表~\ref{tab:sensitivity}、图~\ref{fig:sensitivity} 和表~\ref{tab:tuned_candidates}。 \\
矩阵式复用在高类别合成结构判别中更有优势。 & 表~\ref{tab:synthetic_class_scaling_zh}、图~\ref{fig:synthetic_multiclass_scaling_zh}、表~\ref{tab:synthetic_high_class_zh} 和表~\ref{tab:synthetic_distribution_high_class_zh}。 \\
\bottomrule
\end{tabular}
\end{table}
...
表~\ref{tab:synthetic_distribution_high_class_zh} 按图族报告高类别合成结果。该表放在附录中，因为正文主要聚焦类别数扩展趋势。结果显示，MatrixRes 在 BA、ER 和随机正则图上最强，而 SBM 和 WS 对矩阵式复用并不总是有利。
